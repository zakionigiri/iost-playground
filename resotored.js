...
class VoteContract {
  ...
  initProducer(proID, proPubkey) {
    ...
    blockchain.callWithAuth('vote.iost', 'addOption', [voteId, proID, false])
    const pro = {
      pubkey: proPubkey,
      loc: '',
      url: '',
  ...
  applyRegister(account, pubkey, loc, url, netId, isProducer) {
    ...
    const voteId = this._getVoteId()
    blockchain.callWithAuth('vote.iost', 'addOption', [voteId, account, false])
    blockchain.receipt(
      JSON.stringify([account, pubkey, loc, url, netId, isProducer])
    )
  }
  ...
  unregister(account) {
    ...
    if (this._inCurrentOrPendingList(account)) {
      throw new Error(
        "producer in pending list or in current list, can't unregister"
      )
    }
    const voteId = this._getVoteId()
  ...
  getProducer(account) {
    ...
    if (pro.status === STATUS_APPROVED || pro.status === STATUS_UNAPPLY) {
      const voteId = this._getVoteId()
      pro['voteInfo'] = this._call('vote.iost', 'getOption', [voteId, account])
    }
    return pro
  }
  ...
  _updateCandidateVars(account, amount, voteId, payer) {
    let votes = new Float64(
      this._call('vote.iost', 'getOption', [voteId, account]).votes
    )
    if (amount.gt('0')) {
      ...
      if (amount.lt('0')) {
        ...
        if (votes.lt(VOTE_THRESHOLD)) {
          this._updateCandidateMask(
            account,
            votes.minus(amount).negated(),
            payer
          )
        } else {
          this._updateCandidateMask(account, amount, payer)
  ...
  getVote(voter) {
    const voteId = this._getVoteId()
    return this._call('vote.iost', 'getVote', [voteId, voter])
  }
  ...
  candidateWithdraw(account) {
    ...
    this._topupVoterBonusInternal(
      account,
      new Float64(voterBonus),
  ...
  _getWitnessWatched() {
    ...
    for (const witness in witnessWatched) {
      if (
        witnessWatched[witness].bn + 144 * VOTE_STAT_INTERVAL <
        block.number
      ) {
  ...
  stat() {
    ...
    const preList = []
    const witnessProduced = JSON.parse(
      storage.globalGet('base.iost', 'witness_produced') || '{}'
    )
    ...
    for (const res of voteRes) {
      ...
      if (
        !pro ||
        !pro.isProducer ||
        (pro.status !== STATUS_APPROVED && pro.status !== STATUS_UNAPPLY)
      ) {
        continue
      }
    ...
    for (const key of pendingProducerList) {
      ...
      const forbidUntil = this._updateWitnessPenality(
        witnessPenality,
        witnessWatched,
        account
      )
    ...
    const pendingList = newList.map(x =>
      (function() {
        return x.key
      })()
    )
